...
Bool_t GVertex::CalcSecVTX() {
  ...
  if( gVertex -> GetZCut() ) {
    ...
    for(Int_t i = 0; i < gData->Getnr_secvtx(); i++){
      ...
      if ( fSecvtx_Chi2/fSecvtx_Ndof > 6. /*&& gData->Getvtxsec_x(i)!=666 && gData->Getvtxsec_y(i)!=666 && gData->Getvtxsec_z(i)!=666 && 
               gData->Getvtxredprm_x()!=666 && gData->Getvtxredprm_y()!= 666 && gData->Getvtxredprm_z()!= 666*/  /*&& gHeader->GetAllCuts() && gCards->GetDIMUONGeneral()*/ ) {fChi2Ndof = kFALSE;}
...
Bool_t GVertex::GetAllCuts() {
  ...
  if ( GetZCut() && GetXCut() && GetYCut() && GetXYCut() && GetBeamSpotCut() /*&& !GetChi2NdofCut()*/ )
    return kTRUE;
  else
    return kFALSE;
}
