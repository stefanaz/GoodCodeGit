...
// for (Int_t t = 0;t<520;t++)
// {
// entries = 0;
// cout<<endl;
// for (Int_t i=1;i<=obj[t][0]->GetNbinsX();i++) {entries = entries + obj[t][0]->GetBinContent(i);}
// cout<<obj[t][0]->GetName()<<" Nevnts =  "<<entries<<"   Nbins = "<<obj[histonr][0]->GetNbinsX();
// }
...
// 2 -  h_ptmu_all
// 3 -  h_dimuptmu_all
// 4 -  h_mueta_accept_all
// 5 -  h_ptasym_all
// 6 -  h_dphii_all
// 7 -  h_muissqsum_accept_all
// 8 -  h_thrust_all
// 9 -  h_PtRel_accept_all
// 10 - h_Impact2D_all
// 11 - h_Impact2D_Sign_all
// 12 - h_empz_all
// 13 - h_dimulog10q2si_all
// 14 - h_cal_et_all
// 15 - h_etex2ir_all
// 16 - h_cal_et_m_cal_et_10_all
// 17 - h_dimuphi_all
// 18 - h_dimueta_all
// 19 - h_dimuxsecpt_all
// 20 - h_dimuxseceta_all
// 21 - h_dimuxsecfinders_all
// 22 - h_dimuxsecdr_all
// 23 - h_dimuxsecdphi_all
// 24 - h_dimuxsecptb_all
// 25 - h_dimuxsecptb_sameb_all
// 26 - h_dimuxsecptb_diffb_all
// 27 - h_dimuxsecptb_samecone_all
// 28 - h_dimuxsecptb_asym_all
// 29 - h_mcdimumass1bin_true_hfl_mu_all
// 30 - h_mcdimuxsecPt_true_hfl_mu_all
// 31 - h_mcdimuxsecEta_true_hfl_mu_all
// 32 - h_mcdimuxsecFinders_true_hfl_mu_all
// 33 - h_mcdimuxsecDR_true_hfl_mu_all
// 34 - h_mcdimuxsecDphi_true_hfl_mu_all
// 35 - h_mcdimuxsecPtb_true_hfl_mu_all
// 36 - h_mcdimuxsecPtb_sameb_true_hfl_mu_all
// 37 - h_mcdimuxsecPtb_diffb_true_hfl_mu_all
// 38 - h_mcdimuxsecPtb_samecone_true_hfl_mu_all
// 39 - h_mcdimuxsecPtb_asym_true_hfl_mu_all
// 40 - h_dimuq2si_all
// 41 - h_dimuq2si_cutjpsi_all
// 42 - h_dimulog10q2si_cutjpsi_all
}
...
Int_t nhistoses = count1/13; cout<<nhistoses<<endl;

const Int_t Histo_type = 12;	/// Isolation type
const Int_t Is_type = 4;	/// Isolation type
const Int_t sample = 49;	/// Sample type

TCanvas * canv = new TCanvas("canv","canv",1005,500);
canv->cd(1);
// obj[Histo_type+Is_type*nhistoses][sample]->Rebin(5);
obj[Histo_type+Is_type*nhistoses][sample]->Draw("E1");


entries = 0;
cout<<endl;
for (Int_t i=1;i<=obj[Histo_type+Is_type*nhistoses][sample]->GetNbinsX();i++) {entries = entries + obj[Histo_type+Is_type*nhistoses][sample]->GetBinContent(i);}
cout<<obj[Histo_type+Is_type*nhistoses][sample]->GetName()<<" Nevnts =  "<<entries<<"   Nbins = "<<obj[Histo_type+Is_type*nhistoses][sample]->GetNbinsX()<<endl;

// for(Int_t p=0;p<44;p++) {cout<<obj[p][0]->GetName()<<endl;}
