if [ -z $1 ]; then
    ...
    echo ""
    echo "Usage: getdanny [version], e.g. ./getdanny_dimuon_06e.sh 041011_1_withtrigger_isocorr2"
    echo ""
    exit
fi
#
...
target=$ver'_dimuon_06e_dbothistos/'
echo 'Chosen target directory is: '$target
cd $GUTANADIR/
cd data/
pwd
./oli2danny_dimuon_06e_data $ver da
cd ../mc
./oli2danny_dimuon_06e_mc $ver da
cd $GUTANADIR
mkdir $target
mv 06e.root $target
