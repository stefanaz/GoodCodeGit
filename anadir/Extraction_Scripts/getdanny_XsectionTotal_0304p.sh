if [ -z $1 ]; then
    ...
    echo ""
    echo "Usage: getdanny [version], e.g. ./getdanny_XsectionTotal_0304p.sh 041011_1_withtrigger_isocorr2"
    echo ""
    exit
fi
#
...
target=$ver'_XsectionTotal_0304p_dbothistos/'
echo 'Chosen target directory is: '$target
cd $GUTANADIR/
cd data/
pwd
./oli2danny_XsectionTotal_0304p_data $ver da
cd ../mc
./oli2danny_XsectionTotal_0304p_mc $ver da
cd $GUTANADIR
mkdir $target
