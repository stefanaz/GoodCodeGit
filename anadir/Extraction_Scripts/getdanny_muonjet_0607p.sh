if [ -z $1 ]; then
    ...
    echo ""
    echo "Usage: getdanny [version], e.g. ./getdanny_muonjet_0607p.sh 041011_1_withtrigger_isocorr2"
    echo ""
    exit
fi
#
...
target=$ver'_muonjet_0607p_dbothistos/'
echo 'Chosen target directory is: '$target
cd $GUTANADIR/
cd data/
pwd
./oli2danny_muonjet_0607p_data $ver da
cd ../mc
./oli2danny_muonjet_0607p_mc $ver da
cd $GUTANADIR
mkdir $target
