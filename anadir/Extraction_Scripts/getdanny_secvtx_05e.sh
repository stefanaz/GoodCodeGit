if [ -z $1 ]; then
    ...
    echo ""
    echo "Usage: getdanny [version], e.g. ./getdanny_secvtx_05e.sh 041011_1_withtrigger_isocorr2"
    echo ""
    exit
fi
#
...
target=$ver'_secvtx_05e_dbothistos/'
echo 'Chosen target directory is: '$target
cd $GUTANADIR/
cd data/
pwd
./oli2danny_secvtx_05e_data $ver da
cd ../mc
./oli2danny_secvtx_05e_mc $ver da
cd $GUTANADIR
mkdir $target
mv 05e.root $target
