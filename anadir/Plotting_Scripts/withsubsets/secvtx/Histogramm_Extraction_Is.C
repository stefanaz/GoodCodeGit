...
// gROOT->ProcessLine(".L /data/zenith234d/stefan/GutCode/anadir/mini_HeraII_0307p_Data_MC_Dimuon_Finalversion_dbothistos_dimuon/parameters_set.h");
...
// Set plotting parametes:
  ...
  if(!plot_diffxsec)  Double_t MC_scale_jpsi2 = 0.90*0.036;    // tuning 080116 - for tot. xsec with BHadron ptb PAPERED2, new charm 1.37
...
// initialise histogram array:
  ...
  for(Int_t vorne = 0; vorne<12000; vorne++)
    {
...
//******* FL EXC FUDGED -  WARNING!!!
  ...
  for(Int_t i=startfile; i<nfiles; i++) 
	...
	{// batch101to106_all_histosv04.root
	  if(oldfilestyle)    file = new TFile("doesnotexist.root","READ");
	  else                file = new TFile("instanton.root","READ");
	  TIter next( file->GetListOfKeys() );
	  count1=0;
	  cout <<  file->GetName() <<" : file opened" << endl;
	}
...
/// N.Stefaniuk , modified charm lumi , due to thesis: 
      ...
      if(j==49)  obj[k][j]->Scale(MC_scale_inst); // instanton.root



// add lumi-reweighted beauty histos to first beauty histo sample:

      if(j!=49 && j!=0 && j!=dataposition && (j<charmbegin || j>charmend) && j!=bbbb_position && j!=cccc_position && j!=bbcc_position )
	{ 
	  obj[k][0]->Add(obj[k][j],1); // ccbar MC is added later on below!
	}

//add lumi-reweighted charm histos to first charm histo sample:
      if(j!=49 && j!=0 && j!=dataposition &&  j!=charmbegin && !(j<charmbegin || j>charmend) && j!=BH_position1 && j!=BH_position2 && j!=BH_position3 && j!= jpsi_position1 && j!= jpsi_position2 && j!= psiprime_position1 && j!= psiprime_position2 && j!= ups1_position && j!= ups2_position && j!= ups3_position && j!=bbbb_position && j!=cccc_position && j!=bbcc_position )
	{
  ...
  TH1D* instanton_us[1000];
  TH1D* instanton_ls[1000];
...
instanton_us[plotss] = (TH1D*) obj[histonr][49]->Clone("instanton_us");
instanton_ls[plotss] = (TH1D*) obj[histonr+nhistos][49]->Clone("instanton_ls");
...
// instanton[plotss]
	      ...
	      if(rebinvalue > 0) 
		{
		instanton_us[plotss]->Rebin(rebinvalue); 
		instanton_ls[plotss]->Rebin(rebinvalue); 
		}

	      histoname=obj[histonr][dataposition]->GetName();
	      histoname.Append("_instantons");
	      gInclude->DumpHisto((TH1D*)instanton_us[plotss]->Clone(histoname),histoname);

	      histoname=obj[histonr+nhistos][dataposition]->GetName();
	      histoname.Append("_instantons");
	      gInclude->DumpHisto((TH1D*)instanton_ls[plotss]->Clone(histoname),histoname);
